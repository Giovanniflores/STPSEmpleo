...
@Stateless
@Path("/buscarIdCandidatos")
public class BuscarIdCandidatosEmpresa {

	private Gson gson = new Gson();
	private static Logger logger = Logger.getLogger(BuscarIdCandidatosEmpresa.class);
	...
	@Context HttpServletRequest request, @Context HttpServletResponse response) {
		...
		//		Collections.sort(candidatos,new CandidatoComparador());
				busquedaCandidato.setTamano(indices.size());
			...
			busquedaCandidato.setIdCandidates(indices);
			return gson.toJson(busquedaCandidato);
	...
	public String buscarCandidatos(@DefaultValue("0") @PathParam("tipoConsulta") final String tipoConsulta,
		...
		case 6:
			...
			busquedaCandidato.setIdCandidates(limitarCandidatos(indices,Integer.valueOf(paginaStr),Integer.valueOf(resultadoPorPaginaStr)));
			return gson.toJson(busquedaCandidato);
	...
	private List<Long> limitarCandidatos(List<Long> indices,
			Integer pagina, Integer porPagina) {
		//ordenar la lista
	//	Collections.sort(indices, new BuscarCandidatoComparador());
		List<Long> candidatosPaginador = new ArrayList<Long>();
		if (indices != null && indices.size() > 0) {

			if (indices.size() < ((pagina + 1) * porPagina)) {
				candidatosPaginador = indices.subList(
						pagina * porPagina, indices.size());
			} else {
				candidatosPaginador = indices.subList(
