...
import mx.gob.stps.portal.core.oferta.detalle.service.ServiceLocator;
import mx.gob.stps.portal.core.oferta.detalle.service.ServiceLocatorException;
...
@Stateless
@Path("/portalEmpleoRestWS")
public class PortalEmpleoRestWS {

	private static Logger logger = Logger.getLogger(PortalEmpleoRestWS.class);
...
//	public static final String JNDI_OLA = "OLAService#mx.gob.stps.portal.core.oferta.detalle.service.OLAServiceRemote";

	/**
	 * Default constructor.
	 */
	public PortalEmpleoRestWS() {
	...
	/**
	 * Retrieves representation of an instance of PortalEmpleoRestWS
	 * 
	 ...
	 */
	@GET
	@Path("/consultaOfertasEmpleoPorCarrera/{idCarrera}")
	@Produces({ MediaType.APPLICATION_JSON })
	public OfertasPortalEmpleo consultaOfertasEmpleoPorCarrera(
			@PathParam("idCarrera") int idCarrera) throws BusinessException, ServiceLocatorException {
		logger.info(">>>>>>JGLC>>>>>>>" + idCarrera);
		if (idCarrera <= 0)
			throw new IllegalArgumentException("Identificador de Carrera requerido");
	...
	public OfertasPortalEmpleo consultaOfertasEmpleoPorOcupacion(
			int idOcupacion, String ocupacion) {
	...
	public OfertasPortalEmpleo consultaOfertasEmpleoPorEntidadOcupacion(
			int idEntidad, int idOcupacion, String ocupacion) {

		if (idEntidad <= 0 && idOcupacion <= 0)
			throw new IllegalArgumentException(
					"Identificador de Entidad y Ocupacion requerido");
		if (ocupacion == null || ocupacion.isEmpty())
			ocupacion = "";
		...
		} else if (idEntidad <= 0 && idOcupacion > 0) { // Indicaron solo la
														// ocupacion
	...
	public OfertasPortalEmpleo consultaOfertasEmpleoPorEntidadCarrera(
			int idEntidad, int idCarrera, String carrera) throws BusinessException, ServiceLocatorException {

		if (idEntidad <= 0 && idCarrera <= 0)
			throw new IllegalArgumentException(
					"Identificador de Entidad y Carrera requerido");
		if (carrera == null || carrera.isEmpty())
			carrera = "";
		...
		if (idEntidad > 0 && idCarrera <= 0) { // Indicaron solo la entidad
		...
		} else if (idEntidad <= 0 && idCarrera > 0) { // Indicaron solo la
														// carrera
			...
			List<Integer> idsCarreraPortal = consultaCarrerasEquivalentes(idCarrera);
			if (idsCarreraPortal == null || idsCarreraPortal.isEmpty())
				throw new IllegalArgumentException("Carrera " + idCarrera
						+ " no pertenece al catalogo");
...
//	}
	...
	private List<Integer> consultaCarrerasEquivalentes(int idCarreraOLA) throws BusinessException, ServiceLocatorException {
		List<Integer> idsCarrera = olaFacade.consultaCarrerasOLA(idCarreraOLA);
//		List<Integer> idsCarrera = this.getConsultaCarrerasOLA().consultaCarrerasOLA(idCarreraOLA);
		logger.info(">>>>>>JGLC  idsCarrera.size()>>>>>>>" + idsCarrera.size());
