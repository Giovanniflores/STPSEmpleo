...
/*@Entity
@Table(name = "TERCERA_EMPRESA")
@NamedQueries({
	@NamedQuery(name = "buscaTercerasEmpresas",
			query = "SELECT c FROM TerceraEmpresa c WHERE c.idEmpresa=:idEmpresa AND c.estatus=:estatus ORDER BY c.nombre, c.razonSocial")
})*/
public class TerceraEmpresa implements Serializable {
	private static final long serialVersionUID = -6025047940571419015L;
	...
	/**
	 * Method 'TerceraEmpresa'
	 * 
	 */
	private TerceraEmpresa() {
	}
	 ...
	 */
	@Id
	@SequenceGenerator(name = "SEQ_TERCERA_EMPR", sequenceName = "SEQ_TERCERA_EMPR", allocationSize = 1)
	@GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "SEQ_TERCERA_EMPR")
