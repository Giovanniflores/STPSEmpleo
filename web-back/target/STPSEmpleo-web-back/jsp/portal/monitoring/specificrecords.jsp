...
<%
	...
	String context = request.getContextPath() +"/";
  	...
  	if (null != list && list.size() > 0)
		msg = strTotalNumeroPlazas + " plazas disponibles de " + list.size() + " resultados encontrados para la búsqueda: " + (null != session.getAttribute("SPECIFIC_SEARCH") ? "<span class=\"busqueda_concepto\">" + (String)session.getAttribute("SPECIFIC_SEARCH") + "</span>" : "");
  	else
...
%>
	...
	<table border="0" cellspacing="0" cellpadding="0" class="offer">
		<tr class="temas">
			<th>T&iacute;tulo de la oferta <a id="titulo_orden_asc" style="display: inline;" class="ascendente" href="javascript:orderBy('asc','1')"></a> <a id="titulo_orden_desc" style="display: inline;"  class="descendente" href="javascript:orderBy('desc','1')"></a></th>
			<th>Ubicaci&oacute;n <a id="ubicacion_orden_asc" style="display: inline;" class="ascendente" href="javascript:orderBy('asc','2')"></a> <a id="ubicacion_orden_desc" style="display: inline;"  class="descendente" href="javascript:orderBy('desc','2')"></a></th>
			<th>Empresa <a id="empresa_orden_asc" style="display: inline;" class="ascendente" href="javascript:orderBy('asc','3')"></a> <a id="empresa_orden_desc" style="display: inline;"  class="descendente" href="javascript:orderBy('desc','3')"></a></th>
			<th>Salario neto ofrecido (mensual) <a id="salario_orden_asc" style="display: inline;" class="ascendente" href="javascript:orderBy('asc','4')"></a> <a id="salario_orden_desc" style="display: inline;"  class="descendente" href="javascript:orderBy('desc','4')"></a></th>
			<c:if test="${empty USUARIO_APP || !USUARIO_APP.candidato}">
				<th class="fin">Fecha de publicaci&oacute;n <a id="fecha_orden_asc" style="display: inline;" class="ascendente" href="javascript:orderBy('asc','5')"></a> <a id="fecha_orden_desc" style="display: inline;"  class="descendente" href="javascript:orderBy('desc','5')"></a></th>
			</c:if>
			<c:if test="${not empty USUARIO_APP && USUARIO_APP.candidato}">
				<th>Fecha de publicaci&oacute;n <a id="fecha_orden_asc" style="display: inline;" class="ascendente" href="javascript:orderBy('asc','5')"></a> <a id="fecha_orden_desc" style="display: inline;"  class="descendente" href="javascript:orderBy('desc','5')"></a></th>
	...
	%>
			<tr <% out.print(rowCounter % 2 == 0 ? "" : "class='odd'"); %>/>
				<td><div class="profesion"><strong><a
					href="<%=context%>detalleoferta.do?method=init&id_oferta_empleo=<% out.print(ofertasCanal.getIdOfertaEmpleo()); %>"><% out.print(ofertasCanal.getTituloOferta()); %></a></strong></div></td>
				<td><% out.print(ofertasCanal.getEntidad() + ", " + ofertasCanal.getMunicipio()); %></td>
				<td><% out.print(ofertasCanal.getEmpresa()); %></td>
				<td><% out.print(ofertasCanal.getsSalario()); %></td>
				<td><% out.print(ofertasCanal.getFechaInicioString()); %></td>
				<c:if test="${not empty USUARIO_APP && USUARIO_APP.candidato}">
