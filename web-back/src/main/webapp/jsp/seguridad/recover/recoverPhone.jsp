	...
	function sendMail() {
		...
		if (dijit.byId('correo').value != '') {
			if (dijit.byId('correo').isValid()) {
				...
				message('El correo no es válido, verifique captura');
   			}
		}else {
			dijit.byId('correo').focus();
...
<form name="contrasenaForm" id="contrasenaForm" method="post"
	...
	<input type="hidden" name="correoElectronico" id="correoElectronico" value="${contrasenaForm.correoElectronico}" dojoType="dijit.form.ValidationTextBox"/> 
	...
	<div class="miEspacio formApp">
		...
		<div class="col-md-5" style="float:none;margin:auto;text-align:center">
			...
			<div class="alert alert-info">
				<p>&nbsp;</p>
				<p>Ingrese una dirección de correo electrónico para el envío de recuperación de contraseña<strong class="c_naranja">*</strong></p>
				<p>
					Usuario recuperado: <br/>
					<strong class="c_naranja">${contrasenaForm.usuario}</strong>
				</p>
			</div>
			<div class="row">
				<div class="col-md-12">
					<input type="text" name="correo" id="correo" size="40" maxlength="65" dojoType="dijit.form.ValidationTextBox" class="form-control"
						required="true" regExp="${regexpmail}" invalidMessage="Correo no válido, verificar captura." missingMessage="El correo es requerido."/>
					<p>&nbsp;</p>
				</div>
			</div>
			...
			<div id="divDatosCandidato" class="alert alert-success" style="display: none">
				...
				<p>
					...
					<strong>CURP: </strong>${contrasenaForm.curp}<br /><br /><br />
			...
			<div class="form_nav"><p>&nbsp;</p>
				<input type="button" id="btnAceptar" value="Aceptar" class="boton_naranja  bt_grande" onclick="javascript:sendMail();" /> 
