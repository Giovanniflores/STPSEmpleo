...
<div class="paginator">
	...
	<p class="mostradas" style="font-weight: bold">
		Mostrando
	...
	<c:choose>
		...
		<c:otherwise>
			...
			<li><a class="prev ant" href="javascript:doSubmitPagina(${primeraPaginaBloqueAnterior}, '${sufijo}')">Anterior</a></li>
		</c:otherwise>
	</c:choose>
	<!-- numeración de páginas a mostrar -->
		...
		<c:forEach var="i" begin="${paginaInicialBloque}" end="${paginaFinalBloque}" step="1" varStatus="status">
			<c:choose>
				...
				<c:otherwise>
					<li><a class="pagina" href="javascript:doSubmitPagina(${i}, '${sufijo}')"><c:out value="${i}" /></a></li>
				</c:otherwise>
			</c:choose>
		</c:forEach>
	...
	<c:choose>
		...
		<c:otherwise>
			...
			<li><a class="next sig" href="javascript:doSubmitPagina(${primeraPaginaBloqueSiguiente}, '${sufijo}')">Siguiente</a></li>
		</c:otherwise>
	</c:choose>
	...
	<c:if test="${totalPaginas > 1}">
		<p>
			...
			<input type="text" id="numPagina" name="numPagina"/>
			<input class="ir" type="button" value="Ir" onclick="javascript:doSubmitPagina(getElementById('numPagina').value, '${sufijo}')"/>
		</p>
	</c:if>
