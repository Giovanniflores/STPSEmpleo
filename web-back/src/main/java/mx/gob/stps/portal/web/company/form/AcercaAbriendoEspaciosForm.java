...
import static mx.gob.stps.portal.web.infra.utils.Constantes.CAT_MEDIO_ENTERADO;
import static mx.gob.stps.portal.web.infra.utils.Constantes.CAT_TIPO_EMPRESA;
import static mx.gob.stps.portal.web.infra.utils.Constantes.CAT_ACTIVIDAD_ECONOMICA;
...
public class AcercaAbriendoEspaciosForm extends DomicilioForm implements Serializable{
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 7206613716115966235L;

	private static Logger logger = Logger.getLogger(AcercaAbriendoEspaciosForm.class);
	...
	public ActionErrors validate(ActionMapping mapping, HttpServletRequest request) {
		...
		SimpleDateFormat sdfSource = new SimpleDateFormat("yyyy-MM-dd");
			...
			if (rfc.length()>0){
				String strPatron = "^[A-Za-z\\s\\��0-9]{0,13}$";
				
				if(!Utils.validaPatron(strPatron,rfc)){
					errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.message", "El RFC no es un dato v�lido."));
			    }	
			}
			if (idTipoPersona<=0) {
			...
			if (idTipoPersona==1) {
				...
				if (nombre==null || nombre.length()<1){
				    ...
				    if(!Utils.esAlfabetico(nombre)){				
				    	errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "El Nombre no es un dato v�lido."));
				    }				
				}
				if (apellido1==null || apellido1.length()<2){
				    ...
				    if(!Utils.esAlfabetico(apellido1)){
				    	errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "El Primer Apellido no es un dato v�lido."));
				    }								
				}
				if (fechaNacimiento==null){
				    ...
				    if(!Utils.validaFecha(fechaNacimiento)){
				    	errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "La Fecha de nacimiento no es un dato v�lido."));
				    } else{
		    			try {
				...
				if (razonSocial==null || razonSocial.length()<3){
					errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.required", "La Raz�n social es un dato que debe tener al menos 3 caracteres y "));
				} else {
					//DESCOMENTAR EN QA String strPatron = "^[A-Za-z\\s\\d\\-.&,������������/']{3,50}$";
					//DESCOMENTAR EN Prod String strPatron = "^[A-Za-z\\s\\-.&,������������/']{3,50}$";
					String strPatron = "^[A-Za-z\\s\\d\\-.&,������������/']{3,50}$";
					if(!Utils.validaPatron(strPatron,razonSocial)){				    	
				    	errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "La Raz�n social no es un dato v�lido."));
				    }								
				}	
				if (fechaActa==null){
				    ...
				    if(!Utils.validaFecha(fechaActa)){
				    	errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "La Fecha de acta constitutiva no es un dato v�lido."));
				    } else{
		    			try {
			...
			if (nombreComercial.length()>0){	
				 if(!Utils.esAlfanumerico(nombreComercial)){
				    	errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "El Nombre Comercial no es un dato v�lido."));
				    }	
			}		
			
			
			if(getCodigoPostal()==null || getCodigoPostal().length()<1){
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.required", "El c�digo postal es un dato que"));
			} else {
				int iCodigoPostal_len = getCodigoPostal().length();
			    if(iCodigoPostal_len<5){
			    	errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.message", "El c�digo postal debe tener 5 d�gitos."));
			    } 															
			    if(!Utils.esNumero(getCodigoPostal())){
			    	errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "El c�digo postal no es un dato v�lido."));
			    } 											
			}
			if(getIdEntidad()<=0){
			...
			if(getIdMunicipio()<=0){
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.required", "La delegaci�n o municipio es un dato que"));
			}
			if(getIdColonia()<=0){
			...
			if (getNumeroExterior()==null || getNumeroExterior().length()<1){
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.required", "El n�mero exterior es un dato que"));
			} else {
			    if(!Utils.esAlfanumerico(getNumeroExterior())){
			    	errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "El n�mero exterior no es un dato v�lido."));
			    }							
			}			
			if (getNumeroInterior()!=null && getNumeroInterior().length()>0 && !Utils.esAlfanumerico(getNumeroInterior())){
		    	errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "El n�mero interior no es un dato v�lido."));
			}			
			if (getCalle()==null || getCalle().length()<1){
			    ...
			    if(!Utils.esAlfanumerico(getCalle())){
			    	errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "La calle no es un dato v�lido."));
			    }							
			}				
			if (getEntreCalle()==null || getEntreCalle().length()<1){
			    ...
			    if(!Utils.esAlfanumerico(getEntreCalle())){
			    	errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "Entre calle no es un dato v�lido."));
			    }							
			}	
			if (getyCalle()==null || getyCalle().length()<1){
			    ...
			    if(!Utils.esAlfanumerico(getyCalle())){
			    	errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "Y calle no es un dato v�lido."));
			    }							
			}					
			...
			if (descripcion==null || descripcion.length()<1 || descripcion.length()>2000){
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.required", "La Descripci�n de la empresa es un dato que "));
			} else {
				String strPatron = "^[a-zA-Z������������0-9.,;:#\\s]+$";
				if(!Utils.validaPatron(strPatron,descripcion)){
			    	errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "La Descripci�n de la empresa no es un dato v�lido."));
			    }							
			}
			if (contactoEmpresa==null || contactoEmpresa.length()<1){
				...
				String strPatron = "^[A-Za-z\\s\\������������0-9/']{1,150}$";				
				if(!Utils.validaPatron(strPatron,contactoEmpresa)){
					errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.message", "El Nombre de la persona de contacto con la empresa no es un dato v�lido."));
			    }	
			}
			...
			if (idTipoEmpresa<=0) {
				...
				if(bCheckOption== false){
					errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.message", "Debe seleccionar una opci�n del listado. Por favor no teclee informaci�n en el listado."));
				}				
			}
			if (idActividadEconomica<=0) {
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.required", "La Actividad econ�mica principal es un dato que "));
			} else{
			...
			if (numeroEmpleados<=0){
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.required", "El N�mero de empleados  es un dato que "));
			} else {
			    if(!Utils.esEntero(String.valueOf(numeroEmpleados))){
			    	errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "El N�mero de empleados no es un dato v�lido."));
			    }			
			}
			if (idMedio<=0) {
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.required", "�C�mo se enter� del portal del empleo? es un dato que "));
			} else {
			...
			if (correoElectronico==null || correoElectronico.length()<1){
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.required", "El Correo electr�nico es un dato que "));
			}
			if (correoElectronico!=null && correoElectronico.length()>0 && (confirmarCorreoElectronico==null || confirmarCorreoElectronico.length()==0)){
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.required", "Confirmar correo electr�nico es un dato que "));
			}
			if (correoElectronico!=null && correoElectronico.length()>0 && confirmarCorreoElectronico!=null && confirmarCorreoElectronico.length()>0){
				if (!correoElectronico.equals(confirmarCorreoElectronico)){
					errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.message", "Correo electr�nico y Confirmar correo electr�nico deben ser iguales"));
				} else {
					if(!Utils.validaMail(correoElectronico)){
						errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.message", "El Correo electr�nico no es un dato v�lido."));
				    } else {
						UsuarioDAO usrDAO = UsuarioDAO.getInstance();
						try {
							if(!usrDAO.esCorreoUnico(correoElectronico)){
								errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.message", "El Correo electr�nico proporcionado ya existe."));								
							}
			...
			//TELEFONO PRINCIPAL
			if(idTipoTelefono<1){
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.required", "El tipo de tel�fono es un dato que "));
			}
			if (acceso==null || acceso.length()<1){
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.required", "El acceso del tel�fono  es un dato que "));
			} else {
				if(!Utils.esNumero(acceso)){
					errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "El acceso del tel�fono no es un dato v�lido."));
			    }			
			}
			if (clave==null || clave.length()<1){
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.required", "La Clave Lada del tel�fono  es un dato que "));
			} else {
				if(!Utils.esNumero(clave) || clave.length()>3){
					errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "La Clave Lada del tel�fono no es un dato v�lido."));
			    }						
			}
			if (telefono==null || telefono.length()<1){
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.required", "El Tel�fono es un dato que "));
			} else {
				if(!Utils.esNumero(telefono)){
					errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "El Tel�fono no es un dato v�lido."));
			    } else{
			    	if(clave.length()==2 && telefono.length()!=8){
			    		errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.message", "El tel�fono debe tener 8 d�gitos si la clave Lada tiene 2 d�gitos."));								
			    	}else if(clave.length()==3 && telefono.length()!=7){
			    		errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.message", "El tel�fono debe tener 7 d�gitos si la clave Lada tiene 3 d�gitos."));								
			    	}
			    }
			}
			if (extension!=null && extension.length()>6){
					errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.range", "La extensi�n no es un dato v�lido."));
			}			
