...
public class ComUpdCompanyForm extends DomicilioForm implements Serializable{
	private static final long serialVersionUID = -2047010528645203236L;

	private static Logger logger = Logger.getLogger(ComUpdCompanyForm.class);
	...
	public ActionErrors validate(ActionMapping mapping, HttpServletRequest request) {
		...
		long ID_TIPO_USUARIO = Long.parseLong(request.getSession().getAttribute("ID_TIPO_USUARIO").toString());
		...
		if(ID_TIPO_USUARIO == Constantes.TIPO_USUARIO.ADMINISTRADOR.getIdTipoUsuario()){
			if (idTipoEmpresa<=0) {
		...
		if (correoElectronico!=null && correoElectronico.length()>0 && confirmarCorreoElectronico!=null && confirmarCorreoElectronico.length()>0){
			if (!correoElectronico.equals(confirmarCorreoElectronico)){
				...
				if(!Utils.validaMail(correoElectronico)){
					errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("errors.message", "El Correo electrónico es un dato inválido."));
			    } else {
